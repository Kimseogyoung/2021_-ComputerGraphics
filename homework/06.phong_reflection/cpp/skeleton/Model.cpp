...
void Model::draw(int loc_a_position, int loc_a_normal, int loc_u_ambient, int loc_u_diffuse, int loc_u_specular, int loc_u_shininess,int  loc_u_diffuse_texture, int   loc_a_texcoord)
{
  ...
  glBindTexture(GL_TEXTURE_2D, texid);
    ...
    for (int i = 0; i < mMeshes.size(); ++i)
    {
        ...
        Material material = mMeshes[i].mMaterial;

        glUniform3f(loc_u_ambient, material.ambient.x, material.ambient.y, material.ambient.z);
        glUniform3f(loc_u_diffuse, material.diffuse.x, material.diffuse.y, material.diffuse.z);
        glUniform3f(loc_u_specular, material.specular.x, material.specular.y, material.specular.z);
